...
impl KeyManager {
    ...
    pub fn set_api_key(&self, name: &str, key: &str) -> Result<(), KeyManagerError> {
        Entry::new(&self.service_name, &self.get_credential_name(name))?.set_password(key)?;
        Ok(())
    }

    pub fn get_api_key(&self, name: &str) -> Result<String, KeyManagerError> {
        let key =
            Entry::new(&self.service_name, &self.get_credential_name(name))?.get_password()?;
        Ok(key)
    }

    pub fn delete_api_key(&self, name: &str) -> Result<(), KeyManagerError> {
        Entry::new(&self.service_name, &self.get_credential_name(name))?.delete_credential()?;
        Ok(())
    }
}
